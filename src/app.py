...
from exporter import (
    extract_zip, parse_xml, filter_workout_data, filter_by_date, get_heartrate_for_workout,
    calculate_heartrate_stats, nightly_respiratory_rate_stats, nightly_spo2_stats, daily_step_count,
    daily_resting_hr, vo2max_trend, walking_efficiency, daily_hrv, nightly_temp_deviation,
    daily_met_minutes, daily_energy, add_trimp_column
)
...
data_source = st.sidebar.radio(
    "Select data source",
    ["Upload ZIP/XML", "Load from data/ folder"]
)

zip_file = xml_file = None
folder_xml_path = os.path.join(os.path.dirname(__file__), "data/apple_health_export", "export.xml")
...
if data is not None and workouts is not None:
    ...
    analysis = st.sidebar.radio("Choose analysis", [
        "Overview", "Workouts", "Heart Rate", "Respiratory Rate", "SpO2", "Steps", "Energy", "VO2Max", "HRV", "Temperature", "MET Minutes"
    ])
    st.sidebar.markdown("---")
    ...
    if analysis == "Overview":
        ...
        selected_workout_idx = st.selectbox("Select a workout to view details", filtered.index, format_func=lambda i: f"{filtered.loc[i, 'startDate']} - {filtered.loc[i, 'workoutActivityType']}")
        ...
        hrv_for_workout = hrv_data[(hrv_data['startDate'] >= selected_workout['startDate'].item()) & (hrv_data['endDate'] <= selected_workout['endDate'].item())]
        ...
        rr_for_workout = rr_data[(rr_data['startDate'] >= selected_workout['startDate'].item()) & (rr_data['endDate'] <= selected_workout['endDate'].item())]
        ...
        spo2_for_workout = spo2_data[(spo2_data['startDate'] >= selected_workout['startDate'].item()) & (spo2_data['endDate'] <= selected_workout['endDate'].item())]
        ...
        vo2_for_workout = vo2_data[(vo2_data['startDate'] >= selected_workout['startDate'].item()) & (vo2_data['endDate'] <= selected_workout['endDate'].item())]
        ...
        # Energy per workout
        energy_data = data[data['type'].isin(['ActiveEnergyBurned', 'BasalEnergyBurned'])]
        energy_for_workout = energy_data[(energy_data['startDate'] >= selected_workout['startDate'].item()) & (energy_data['endDate'] <= selected_workout['endDate'].item())]
