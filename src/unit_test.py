...
class TestExporter(unittest.TestCase):
    ...
    def test_filter_by_date(self):
        ...
        # Filter workouts
        filtered_workouts = exporter.filter_by_date(
            self.sample_workouts, start_date, end_date
        )
        ...
        self.assertTrue(
            all(
                filtered_workouts["creationDate"].dt.date
        ...
        expected_count = len(
            self.sample_data[
                self.sample_data["creationDate"].dt.date
                == pd.Timestamp("2024-01-01").date()
            ]
        )
    ...
    def test_trimp_score(self):
        ...
        trimp = exporter.trimp_score(
            duration_min=30.0, hr_avg=150.0, hr_rest=60.0, hr_max=190.0, gender="male"
        )
        ...
        female_trimp = exporter.trimp_score(
            duration_min=30.0, hr_avg=150.0, hr_rest=60.0, hr_max=190.0, gender="female"
        )
